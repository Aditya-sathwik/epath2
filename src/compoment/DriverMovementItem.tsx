...
const DriverMovementItem = (props: Props) => {
  const { data } = props;
    ...
    <TouchableOpacity style={styles.mainView} onPress={props.onPress}>
      ...
      <View style={[styles.rowView, { paddingVertical: 8 }]}>
        ...
        >
          ...
          >
            <Image source={icons.sportscar} style={styles.UserIcon} />
            <View style={styles.meneView}>
     ...
     {data?.status === "upcoming" && <TouchableOpacity onPress={props.onCancelPress} style={styles.btnStyle}>
      <Text style={[styles.btnText]}>{"CANCEL MOVEMENTS"}</Text>
      </TouchableOpacity>}
    </TouchableOpacity>
  );
};

export default DriverMovementItem;
